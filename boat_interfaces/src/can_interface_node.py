...
#############################################################################################
# NOTE: You probably don't want to use this file. It does not match our available hardware. #
#############################################################################################

# TODO: Ensure frame size does not exceed 64 bytes.
# Also double check we can use CAN-FD; If not, we gotta cut stuff WAY down, 8 bytes per frame. and rip strings
...
def parse_msg_config(config):
    ...
    if not 'topic_type' in config:
        config['topic_type'] = rostopic._get_topic_type(config['topic'])[0]
    config['msg_type'] = get_msg_from_type(config['topic_type'])
    ...
    if not 'format' in config:
        config['format'] = parse_msg_format(config['msg_type'])
...
def CAN_read(msg):
    rospy.logdebug("CAN rx: {}".format(msg))

    # If the message is directed to the Odroid and is known:
    if msg.arbitration_id in input_msgs:
        config = input_msgs[msg.arbitration_id]
        ...
        for i, f in enumerate(config['format']):
            ...
            data.append(struct.unpack('!' + f, msg.data[cur_bit:cur_bit + data_len])[0])
            cur_bit += data_len
        ...
        if config['msg_type']._has_header:
            data[config['msg_type']._slot_types.index(
                'std_msgs/Header')].stamp = rospy.Time().from_sec(msg.timestamp)
...
# Write the specified payload to the CAN bus
def CAN_write(id, data):

    msg = can.Message(arbitration_id=id, data=data, timestamp=rospy.Time().to_sec())
    bus.send(msg)
    rospy.logdebug("CAN tx: {}".format(msg))


# Callback for receiving messages from ROS topics, to be forwarded to the CAN bus
def subscriber_callback(data, args):
    id = args[0]
    format = args[1]
    ...
    format = format.format(*string_lengths)
    CAN_write(id, struct.pack("!" + format, *packed_data))
...
def initialize_node():
    ...
    for _, config in bus_config['output_msgs'].items():
        config = parse_msg_config(config)

        rospy.Subscriber(config['topic'], config['msg_type'], subscriber_callback,
    ...
    for _, config in bus_config['input_msgs'].items():
        ...
        pub = rospy.Publisher(config['topic'], config['msg_type'], queue_size=10)
        input_msgs[config['id']] = {
            'publisher': pub,
    ...
    # Setup async CAN bus reader callback
    notifier = can.Notifier(bus, [CAN_read])
