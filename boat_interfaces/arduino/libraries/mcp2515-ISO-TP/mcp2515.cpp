...
MCP2515::ERROR MCP2515::reset(void)
{
    ...
    modifyRegister(MCP_RXB0CTRL,
                   RXBnCTRL_RXM_MASK | RXB0CTRL_BUKT_MASK, RXB0CTRL_BUKT_MASK);
    modifyRegister(MCP_RXB1CTRL, RXBnCTRL_RXM_MASK, 0);
...
{
    return setMode(CANOperationMode::CONFIG);
}

MCP2515::ERROR MCP2515::setListenOnlyMode()
{
    return setMode(CANOperationMode::LISTENONLY);
}

MCP2515::ERROR MCP2515::setSleepMode()
{
    return setMode(CANOperationMode::SLEEP);
}

MCP2515::ERROR MCP2515::setLoopbackMode()
{
    return setMode(CANOperationMode::LOOPBACK);
}

MCP2515::ERROR MCP2515::setNormalMode()
{
    return setMode(CANOperationMode::NORMAL);
}

MCP2515::ERROR MCP2515::setMode(const CANOperationMode mode)
{
    uint8_t val = (uint8_t)(mode);
    modifyRegister(MCP_CANCTRL, CANCTRL_REQOP_MASK, val);

    unsigned long endTime = millis() + 10;
    bool modeMatch = false;
    while (millis() < endTime) {
        uint8_t newmode = readRegister(MCP_CANSTAT);
        newmode &= CANSTAT_OPMOD_MASK;

        modeMatch = newmode == val;
...
void MCP2515::prepareId(uint8_t *buffer, const bool ext, const uint32_t id)
{
    ...
    if (ext) {
        ...
        buffer[MCP_SIDL] |= SID_EXIDE_MASK;
...
MCP2515::ERROR MCP2515::sendFrame(const TXBn txbn, const struct can_frame *frame)
{
...
MCP2515::ERROR MCP2515::sendFrame(const struct can_frame *frame)
{
    ...
    for (uint8_t i=0; i<N_TXBUFFERS; i++) {
        ...
        if ( (ctrlval & TXBnCTRL_TXREQ) == 0 ) {
            return sendFrame(txBuffers[i], frame);
        }
    }
...
MCP2515::ERROR MCP2515::readFrame(const RXBn rxbn, struct can_frame *frame)
{
    ...
    if ( (tbufdata[MCP_SIDL] & SID_EXIDE_MASK) ==  SID_EXIDE_MASK ) {
...
MCP2515::ERROR MCP2515::readFrame(struct can_frame *frame)
{
    ERROR rc;
    uint8_t stat = getStatus();

    if ( stat & STAT_RX0IF_MASK ) {
        rc = readFrame(RXB0, frame);
    } else if ( stat & STAT_RX1IF_MASK ) {
        rc = readFrame(RXB1, frame);
    } else {
        rc = ERROR_NOMSG;
    }
