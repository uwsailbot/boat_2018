...
class SearchAreaSection:
    ...
    def add_child(self, child_center, search_area):
        if self.recursive_level > 1 or (child_center.x - search_area.center.x)**2 + (
                child_center.y - search_area.center.y)**2 < search_area.radius**2:
            self.children.append(
                SearchAreaSection(child_center, self.length / 2.0, self.recursive_level - 1,
                                  search_area))
    ...
    def _draw(self, camera):
        if self.recursive_level is 0 or self.full:
            glColor4f(self.recursive_level * 0.2, self.recursive_level * 0.2,
                      self.recursive_level * 0.2, 0.3)
...
class SearchArea:
    ...
    def setup_coverage(self, resolution=3):
        if self.center is None or self.radius is 0 or self.target is None:
            print(
                "Tried to setup coverage check for search but search area was not completely setup (c:%s r:%s t:%s)"
                % (self.center, self.radius, self.target))
            return
...
)  # in gps, first point is center of circle, second is on edge, record of ros data
...
def waypoints_callback(newPoints):
    ...
    if (len(waypoint_gps.points) - 1) is len(
            newPoints.points) and sound and cur_boat_img is boat_imgs["mars"]:
...
def mouse_handler(button, mouse_state, x, y):
    ...
    if button == GLUT_RIGHT_BUTTON:
    ...
    elif cur_slider is () and (
            sim_mode is SimMode.DEFAULT or sim_mode is SimMode.CONTROLLER
    ) and state.challenge is not BoatState.CHA_STATION and state.challenge is not BoatState.CHA_SEARCH and (
            button == GLUT_LEFT_BUTTON or button == GLUT_MIDDLE_BUTTON):
        newPt = Point()
        ...
        waypoint_pub.publish(waypoint_gps)
            ...
            sim_mode is SimMode.DEFAULT or sim_mode is SimMode.CONTROLLER
    ) and state.challenge is BoatState.CHA_STATION and button == GLUT_LEFT_BUTTON:
        newPt = Point()
        (lps_x, lps_y) = camera.screen_to_lps(x, y)
        newPt.x = lps_x
        newPt.y = lps_y
        ...
        waypoint_pub.publish(waypoint_gps)
            ...
            sim_mode is SimMode.DEFAULT or sim_mode is SimMode.CONTROLLER
    ) and state.challenge is BoatState.CHA_SEARCH and button == GLUT_LEFT_BUTTON:
        (lps_x, lps_y) = camera.screen_to_lps(x, y)
...
def draw_target_point():
    if target_point is not (
    ) and state.major is BoatState.MAJ_AUTONOMOUS and state.minor is not BoatState.MIN_COMPLETE:
        glColor3f(1, 1, 1)
...
def draw_detailed_status():
    ...
    draw_text("output: %.1f" % rudder_output, panel_width / 2, win_height * rudder_offset - 20,
              'center')
    draw_text("input: %.1f" % rudder_input, panel_width / 2, win_height * rudder_offset - 35,
              'center')
    draw_text("setpoint: %.1f" % rudder_setpoint, panel_width / 2, win_height * rudder_offset - 50,
              'center')
    draw_text("enable: %s" % rudder_enable, panel_width / 2, win_height * rudder_offset - 65,
              'center')

    # Draw gps_raw values:
    draw_text("GPS Raw:", panel_width / 2, win_height * gps_offset, 'center', 18)
    draw_text("status: %f" % replay_gps_raw.status, panel_width / 2, win_height * gps_offset - 20,
              'center')
    draw_text("satellites_used: %.1f" % replay_gps_raw.satellites_used, panel_width / 2,
              win_height * gps_offset - 35, 'center')
    draw_text("lat: %.000000001f" % replay_gps_raw.latitude, panel_width / 2,
              win_height * gps_offset - 50, 'center')
    draw_text("long: %.000000001f" % replay_gps_raw.longitude, panel_width / 2,
              win_height * gps_offset - 65, 'center')
    draw_text("alt: %.0000001f" % replay_gps_raw.altitude, panel_width / 2,
              win_height * gps_offset - 80, 'center')
    draw_text("track: %.0000001f" % replay_gps_raw.track, panel_width / 2,
              win_height * gps_offset - 95, 'center')
    draw_text("speed: %.001f" % replay_gps_raw.speed, panel_width / 2,
              win_height * gps_offset - 110, 'center')
    draw_text("hdop: %.01f" % replay_gps_raw.hdop, panel_width / 2, win_height * gps_offset - 125,
              'center')
...
def draw_boat():
    ...
    draw_image(
        ...
        (0, (-cur_boat_img[1][1] / 2 + cur_rudder_img[1][1] * 0.1) *
         camera.scale),  # local y coord of rudder, moves to end of boat
        90 - rudder_pos,  # rudder pos
        (cur_rudder_img[1][0] * camera.scale,
         cur_rudder_img[1][1] * camera.scale))  # rudder visual size
    glPopMatrix()

    #draw boat
    draw_image(cur_boat_img[0], (x, y), heading - 90,
    ...
    glTranslatef(x, y, 0)
    glRotatef(heading - 90, 0, 0, 1)
    draw_image(cur_sail_img[0], (0.1 * camera.scale, 0.5 * camera.scale), sail_angle,
...
def draw_status_boat(x, y):
    ...
    draw_image(cur_boat_img[0], (x, y + 15), 0,
               (cur_boat_img[1][0] * scale, cur_boat_img[1][1] * scale))

    # draw rudder
    rudder_scale = 26.0 / cur_rudder_img[1][1]
    draw_image(cur_rudder_img[0], (x, y - 10), 90 - rudder_pos,
               (cur_rudder_img[1][0] * rudder_scale, cur_rudder_img[1][1] * rudder_scale))

    sail_scale = 42.0 / cur_sail_img[1][1]
    sail_angle = 90 * float(WINCH_MAX - winch_pos) / (WINCH_MAX - WINCH_MIN)
    draw_image(cur_sail_img[0], (x + 1, y + 20), sail_angle,
...
def init_sliders():
    global sliders
    wind_speed_slider = Slider(win_width - 100, win_height - 160, 80, 25,
                               wind_speed_slider_callback, 0, 15, 5)
    wind_speed_slider.set_color(0, 0, 0)
    sliders["Wind speed"] = wind_speed_slider

    sim_speed_slider = Slider(win_width - 100, win_height - 200, 80, 25, sim_speed_slider_callback,
                              0, 1000, 200)
