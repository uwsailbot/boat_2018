...
class StationPlanner(Planner):
    ...
    def planner(self):
        ...
        if self.station_timeout:
            if self._boat_reached_target():
                ...
                rospy.loginfo(rospy.get_caller_id() +
                              " Exited box. Boat State = 'Autonomous - Complete'")
            return
        ...
        if (len(self.box) is not 4 and self.state.minor is not BoatState.MIN_COMPLETE):
            self._set_minor_state(BoatState.MIN_COMPLETE)
            rospy.loginfo(rospy.get_caller_id() +
                          " Box is invalid. Boat State = 'Autonomous - Complete'")
            return
    ...
    def _timer_callback(self, event):
        ...
        final_point = {
            0: Point(box[0].x - tol_dist, cur_pos_gps.y),
        ...
        rospy.loginfo(rospy.get_caller_id() + " Exiting bounding box through: " +
                      final_direction[i])
        ...
        for i in range(4):
            ...
            if abs(inner_box[(i + 1) % 4].x - inner_box[i].x) > 0.0001:
                ...
                if abs(m - m_box) > 0.0001:
                    ...
                    if (x_int <= max(inner_box[i].x, inner_box[(i + 1) % 4].x)
                            and x_int >= min(inner_box[i].x, inner_box[(i + 1) % 4].x)
                            and x_int >= start_point.x
                            and y_int <= max(inner_box[i].y, inner_box[(i + 1) % 4].y)
                            and y_int >= min(inner_box[i].y, inner_box[(i + 1) % 4].y)):
                        intersections += 1
            # Perfectly vertical line, will intersect as long as it is on the right side of the starting point and in the right y range
            else:
                if (inner_box[i].x >= start_point.x
                        and start_point.y <= max(inner_box[i].y, inner_box[(i + 1) % 4].y)
