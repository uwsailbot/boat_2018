...
def waypoints_callback(new_waypoints):
    ...
    if (Planner.state.major is BoatState.MAJ_AUTONOMOUS
            and (Planner.state.minor is BoatState.MIN_COMPLETE
                 or Planner.state.minor is BoatState.MIN_INITIALIZE)
            and len(Planner.waypoints) > 0):
